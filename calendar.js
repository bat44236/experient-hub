...
const CAL = (() => {
  ...
  // ── Google Calendar — plain fetch, no gapi library needed ───────────────
  ...
  async function fetchEvents(calEntries) {
    const timeMin = encodeURIComponent(new Date(calYear, 0,  1).toISOString());
    const timeMax = encodeURIComponent(new Date(calYear, 11, 31, 23,59,59).toISOString());
    store = {};
    await Promise.all(calEntries.map(async entry => {
      try {
        const calId = encodeURIComponent(entry.id);
        const url   = `https://www.googleapis.com/calendar/v3/calendars/${calId}/events`
          + `?key=${currentApiKey}&timeMin=${timeMin}&timeMax=${timeMax}`
          + `&singleEvents=true&orderBy=startTime&maxResults=500`;
        const resp = await fetch(url);
        if (!resp.ok) { console.warn('Calendar fetch', resp.status, entry.id); return; }
